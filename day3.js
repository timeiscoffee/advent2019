...
function findPath(input) {
    ...
    cmds.forEach(cmd=>{
        var dir = cmd.substr(0, 1);
        var size = Number(cmd.substr(1));
        for (var i = 0; i < size; i++) {
            if (dir === 'U') {
                current = [current[0] + 1, current[1]];
                coords.push(current.toString());
            } else if (dir === 'R') {
...
var resultA = findPath(a);
var resultB = findPath(b);

var intersects = resultA.filter(item=>resultB.includes(item))

var smallest = [null, null];
intersects.forEach(intersect=>{
    var result = resultA.indexOf(intersect) + resultB.indexOf(intersect);
    if (smallest[0] === null || smallest[0] > result) {
        smallest = [result, intersect];
    }
}
);
